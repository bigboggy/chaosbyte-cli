...
// BlitzCascade is the slow, deliberate dancing cascade used for the
// moderator's game beats during a /blitz round: target announcement,
// score confirmation, winner reveal. Three phases:
//
//   1. Pre-scramble (0..1200ms): every cell rapid-cycles through the
//      long-ramp glyphs. The whole line reads as alive.
//   2. Cascade settle (1200ms..3000ms): the settle frontier sweeps
//      left to right at split-flap pace; each letter "lands" visibly
//      before its neighbour does.
//   3. Steady (>3000ms): the line is its target, plain.
//
// 3 seconds total. Long enough to read as an event, short enough that
// the player can react to a target word before the cascade resolves.
// This is the macro the room sees fire when something matters.
func BlitzCascade() Macro {
	const scrambleMs = 1200
	const cascadeMs = 1800
	const totalMs = scrambleMs + cascadeMs
	return func(state *AnimationState, layout *Layout, elapsed time.Duration, _ time.Time) bool {
		if elapsed < time.Duration(scrambleMs)*time.Millisecond {
			Scramble(state, elapsed, scrambleMs)
