...
//
// The paint callback is the hook for the blitz dance state: when a round
// is in flight, blitz.Paint mutates the AnimationState so every visible
// chat line enters the cascade wave. Mod ChatActions during the round
// additionally get the longer BlitzCascade macro so each game beat lands
// as a dramatic split-flap moment on top of the room-wide dance.
func renderChatLineAnimDetailed(msg ui.ChatMessage, width int, now time.Time, blitzActive bool, paint func(*typo.AnimationState)) ([]string, string, int) {
	...
	// During a blitz round, mod ChatActions are the game's beats
	// (target announce, score +N, winner reveal). Swap the default
	// 1-second Settle for the longer dancing BlitzCascade so each
	// beat reads as an actual cascade rather than a brief scramble.
	// Player ChatNormal messages stay plain; the cascade is reserved
	// for the moments that matter.
	if blitzActive && msg.Kind == ui.ChatAction {
		macro = typo.BlitzCascade()
	}
	// Extend the arrival window during a blitz so the longer cascade
	// has time to play through.
	window := arrivalWindow
	if blitzActive && msg.Kind == ui.ChatAction {
		window = 4 * time.Second
	}
	if macro != nil && elapsed >= 0 && elapsed < window {
	...
	// Blitz dance state runs last so it overrides the per-message macro
	// when a round is active. Without this the arrival window for joins
	// or actions would fight the room-wide cascade.
	if paint != nil {
		paint(&state)
	}
